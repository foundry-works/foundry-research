...
13. `${CLAUDE_SKILL_DIR}/state audit` — check coverage, identify gaps, get methodology stats
14. **Pre-report gap checkpoint — resolve or justify every open gap.** Review all open gaps from the audit. For each gap, you must either:
...
16. **Synthesis — writer → reviewer → verifier flow.** You are the supervisor. Do NOT write the report yourself. Instead, orchestrate the three synthesis agents:
