...
**Provider diversity matters.** After your search rounds, check provider distribution — if any single provider accounts for >60% of total sources, your bibliography is shaped by that provider's corpus biases and ranking algorithms. Deliberately run follow-up searches on underrepresented providers, especially for questions where your primary provider returned few or weak results. For example, if Semantic Scholar dominates, add targeted OpenAlex or PubMed searches for the weakest-covered questions.
