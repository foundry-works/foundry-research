...
5. `${CLAUDE_SKILL_DIR}/state download-pending --auto-download` — download all sources with DOIs. If the response includes `sync_failures`, run `${CLAUDE_SKILL_DIR}/download --retry-sync` to recover any sources that downloaded but failed to update state.db.
6. Spawn reader subagents for downloaded papers (parallel, one source per agent). As readers complete, log gaps immediately for any research question where you notice thin or conflicting evidence — don't wait until all readers finish. Early gap detection drives targeted follow-up searches while you still have search budget.
...
11. **Pre-report gap checkpoint:** If the audit shows zero gaps logged, pause and justify in journal.md. Zero gaps across 15+ sources almost always means gaps weren't tracked, not that coverage is perfect. Review each research question: does every one have 2+ supporting sources with on-disk content? If not, `log-gap` before proceeding. If coverage genuinely is complete, write a brief journal entry explaining why (e.g., "All 5 questions covered by 3+ sources each, no conflicting evidence found").
