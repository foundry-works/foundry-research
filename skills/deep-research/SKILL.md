...
1. `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state init --query "..." --session-dir ./deep-research-{topic}` — creates session (auto-discovers session dir for all subsequent commands)
...
3. **Delegate brief writing to the brief-writer agent.** Spawn a `brief-writer` subagent (Opus) with the user's query, assumption surfacing results, and session directory path. The agent generates 3-7 research questions including at least one tradeoffs question (what would experts argue about?) and one adversarial question (what's wrong with the obvious answer?). It writes `brief.json` to the session directory. After it returns, load the brief: `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state set-brief --from-json brief.json`.
...
3b. **Optional evidence policy.** If the brief or early source landscape calls for explicit support calibration, write a short `evidence-policy.yaml` file in the session root. This is advisory and run-local; do not create one by default for routine topics, and never treat its absence as a blocker. Use these v1 fields:
   ...
   ```
   Run `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state support-context` after creating or revising the policy and pass the returned support context to downstream agents when relevant. If no policy exists, the command still returns valid context with `evidence_policy.present: false`.
4. **Delegate source acquisition to the `source-acquisition` agent.** Spawn a `source-acquisition` subagent (Opus, foreground) with:
   ...
   - The CLI directory path (`${CLAUDE_PLUGIN_ROOT}/skills/deep-research`)
   ...
   **4b. Validate the acquisition manifest before proceeding.** The acquisition agent runs in a separate context. Before moving to triage and reading:
   1. Run `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state download-pending` (dry run) — check that the `remaining` count matches the manifest's claim.
   ...
   **If the manifest reports `tavily_available: false`:** Check `perplexity_available`, `linkup_available`, `gensee_available`, and `exa_available`. If any is available, the acquisition agent already used it as the web search fallback — no manual compensation needed. If all five web search providers are down (`tavily_available: false`, `perplexity_available: false`, `linkup_available: false`, `gensee_available: false`, `exa_available: false`), compensate immediately — don't wait for gap-mode:
   ...
   3. **Download promising results.** For each useful URL, run `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/download <src-id> --url <url>` to ingest it into the pipeline.
...
6. **Batch pre-read validation (mandatory).** Before spawning any reader agent, validate every candidate source:
   ...
   - If off-topic because the downloaded content is the wrong source → `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state set-quality --id src-NNN --quality title_content_mismatch` and skip
...
11b. **Abstract-based findings (supplement, not fallback).** After deduplication, scan abstract-only sources (`quality: "abstract_only"` or no content file in `sources/metadata/src-NNN.json`) for empirical results that directly address a research question — a sample size, effect, or conclusion, not just topical overlap.
    ...
    2. If the abstract contains a clear empirical result, log a finding directly via `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state log-finding` with the caveat "(abstract only; methodology not verified)" appended to `--text`
...
14. **Delegate gap resolution and applicability searches to the source-acquisition agent (gap mode).** Review all open gaps from the audit. If the audit shows zero gaps logged across 15+ sources, pause — zero gaps almost always means gaps weren't tracked, not that coverage is perfect. Review each research question and `log-gap` for any with < 2 supporting sources.
    ...
    **After the agent returns — verify before resolving gaps.** The source-acquisition agent reports gaps as "potentially resolved" because it downloaded sources matching gap terms, but it cannot verify whether the content actually addresses the gap. Metadata-content mismatches (e.g., a paper titled "multi-informant validity" that actually contains gastroenterology content) mean downloaded ≠ relevant. To avoid false confidence in coverage:
    ...
    5. Run `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state audit` again to confirm coverage actually improved.
...
15. **Synthesis — draft the report.** You are the supervisor. Do NOT write the report yourself. Instead, hand off to the synthesis-writer agent.
    ...
    **c0. Validate report grounding while `draft.md` still exists.** Run:
    ```bash
    ${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state validate-report-grounding --manifest {session_dir}/report-grounding.json --report {session_dir}/draft.md
    ```
...
**JSON input:** Pass JSON via `--from-json FILE` (write to a temp file first) or `--from-stdin` (pipe JSON via stdin). There is no `--json` flag — inline JSON breaks on special characters in titles/abstracts. Example: `echo '{"scope":"..."}' | ${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state set-brief --from-stdin`
...
**A research brief sharpens everything.** A structured brief — scope, key aspects, 3-7 concrete research questions, what a complete answer looks like — drives better searches and becomes the report skeleton. Save it with `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state set-brief`.

**Evidence policy is optional calibration.** When the brief raises high-stakes, fast-changing, quantitative, legal, regulatory, scientific, or source-authority-sensitive questions, a short `evidence-policy.yaml` in the session root can state what source standards and inference tolerance should apply. The policy is local to that session and advisory: it helps agents decide where to be stricter, but it is not a mandatory phase and no command should fail because it is missing. Use `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state support-context` as the single formatter for passing policy context to acquisition, readers, synthesis, verification, and review prompts.
...
**Completion signals:** saturation (repeated results), coverage (every research question has 2-3+ sources), and diminishing returns (tangential results). Simple factual lookups need 3-5 sources, not 30. `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state log-finding` and `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state log-gap` track coverage persistently.
...
**Structured coverage tracking.** Searches and sources are auto-tracked by `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/search`. Findings are logged by the `findings-logger` subagents (step 10) — you do not call `log-finding` directly. **You must call `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state log-gap` for every research question that has fewer than 2 supporting sources** — this is not optional. These persist across context compressions and make `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state summary` actionable — without them, the summary shows empty findings/gaps arrays. **Use the full question text from the brief in `--question`** (e.g., `--question "Q1: What mechanisms drive X?"`) — audit matches findings to brief questions, so abbreviated labels like bare "Q1" may cause false sparse-coverage warnings.
...
You are the supervisor. Your job is to orchestrate subagents and interpret their manifests — not to run searches or read papers yourself. Run `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state` commands directly for lightweight operations (init, mark-read, log-gap, audit, summary). Delegate everything else to subagents.
...
- **Pre-read check (step 6):** This is the mandatory batch validation step from the workflow. Read the first 30 lines of each candidate source's content file. Skip anything off-topic because it is the wrong downloaded source, garbled, or stub — mark wrong-source content via `${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state set-quality --id src-NNN --quality title_content_mismatch`. Use source caution flags for secondary/stale/self-interested/undated/low-relevance concerns. See step 6 for the full rationale and cost justification.
...
for src in src-003 src-035 src-042; do
  ${CLAUDE_PLUGIN_ROOT}/skills/deep-research/state mark-read --id "$src"
done
```
