...
- **`agents` and `skills` in `plugin.json` override defaults.** If omitted, the default `agents/` and `skills/` directories are auto-discovered. Only set them explicitly if you need custom paths.
